...
async function getStats() {
  ...
  const [
    ...
    db.user.findMany({
      ...
      take: 10,
      select: { id: true, name: true, email: true, plan: true, createdAt: true, emailVerified: true },
    }),
  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-8 space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Total Sites" value={stats.totalSites.toLocaleString()} />
        <StatCard label="Active Subscriptions" value={stats.activeSubscriptions.toLocaleString()} />
        <StatCard label="MRR (est.)" value={`$${stats.mrr.toLocaleString()}`} highlight />
      </div>

      {/* Events */}
      <div className="grid grid-cols-2 gap-4">
        <StatCard label="Events This Month" value={stats.eventsThisMonth.toLocaleString()} />
        <StatCard label="Total Events (all time)" value={stats.totalEvents.toLocaleString()} />
      </div>
          ...
          {(["free", "small", "medium", "large"] as const).map((plan) => {
            ...
            return (
              <div key={plan} className="space-y-2">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-600">{pct}% of users</p>
              </div>
            );
          })}
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-800">
                <th className="text-left py-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">User</th>
                <th className="text-left py-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Plan</th>
                <th className="text-left py-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Verified</th>
                <th className="text-left py-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Joined</th>
                <th className="text-left py-2 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {stats.recentUsers.map((user) => (
                <tr key={user.id}>
                  ...
                  <td className="py-2.5 pr-4">
                    <span className={`text-xs font-medium ${user.emailVerified ? "text-green-600 dark:text-green-400" : "text-amber-600 dark:text-amber-400"}`}>
      ...
      {stats.recentSubscriptions.length > 0 && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6">
          <h2 className="font-semibold text-gray-900 dark:text-white mb-4">Recent Subscriptions (30 days)</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-800">
                  <th className="text-left py-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">User</th>
                  <th className="text-left py-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Plan</th>
                  <th className="text-left py-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Status</th>
                  <th className="text-left py-2 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Renews</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                {stats.recentSubscriptions.map((sub) => (
                  <tr key={sub.id}>
                    <td className="py-2.5 pr-4">
                      <p className="font-medium text-gray-900 dark:text-white">{sub.user.name ?? "—"}</p>
                    ...
                    <td className="py-2.5 text-gray-500 dark:text-gray-400">
                      {new Date(sub.currentPeriodEnd).toLocaleDateString("en", { month: "short", day: "numeric", year: "numeric" })}
                    </td>
                  </tr>
                ))}
              </tbody>
...
function StatCard({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`rounded-2xl border p-5 ${highlight ? "bg-blue-600 border-blue-600 text-white" : "bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800"}`}>
      <p className={`text-xs font-medium uppercase tracking-wide mb-1 ${highlight ? "text-blue-200" : "text-gray-500 dark:text-gray-400"}`}>
        {label}
      </p>
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    past_due: "bg-amber-100 dark:bg-amber-900 text-amber-700 dark:text-amber-300",
    paused: "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400",
  };
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${styles[status] ?? styles["paused"]}`}>
      {status.replace("_", " ")}
    </span>
  );
}
