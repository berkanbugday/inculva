...
export default async function BillingPage() {
  ...
  const proMonthly = LS_PRODUCTS.find(
    (p) => p.plan === "pro" && p.interval === "month",
  );
  const proAnnual = LS_PRODUCTS.find(
    (p) => p.plan === "pro" && p.interval === "year",
  );
  const bizMonthly = LS_PRODUCTS.find(
    (p) => p.plan === "business" && p.interval === "month",
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-[#1a1a2e] rounded-3xl shadow-sm p-5 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-1 max-w-xs">
          ...
          />
          <button className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-full transition-colors cursor-pointer">
            Apply
          </button>
        </div>
      </div>
      ...
      {plan !== "business" && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white dark:bg-[#1a1a2e] rounded-3xl shadow-sm p-6">
            ...
            <PlanCheckoutButton
              variantId={proMonthly?.variantId ?? ""}
              label="Continue"
            />
          </div>
          ...
          <div className="bg-white dark:bg-[#1a1a2e] rounded-3xl shadow-sm p-6">
            ...
            <PlanCheckoutButton
              variantId={bizMonthly?.variantId ?? ""}
              label="Continue"
            />
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-[#1a1a2e] rounded-3xl shadow-sm p-6">
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center leading-relaxed">
          Subscription plans are structured based on your website&apos;s monthly
          traffic. Events and usage limits vary by plan — upgrade to unlock
          higher limits and additional features for your accessibility widget.
        </p>
      </div>
