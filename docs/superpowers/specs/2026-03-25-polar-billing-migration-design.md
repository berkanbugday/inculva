...
**New field:** `interval` must be stored to distinguish monthly/yearly subscribers for correct MRR calculation and billing UI display.
...
POLAR_SMALL_YEARLY_PRODUCT_ID
POLAR_MEDIUM_MONTHLY_PRODUCT_ID
POLAR_MEDIUM_YEARLY_PRODUCT_ID
POLAR_LARGE_MONTHLY_PRODUCT_ID
POLAR_LARGE_YEARLY_PRODUCT_ID
```
...
const PRODUCT_TO_PLAN: Record<string, Plan> = {
  ...
  [process.env.POLAR_SMALL_YEARLY_PRODUCT_ID ?? "~~"]: "small",
  [process.env.POLAR_MEDIUM_MONTHLY_PRODUCT_ID ?? "~~~"]: "medium",
  [process.env.POLAR_MEDIUM_YEARLY_PRODUCT_ID ?? "~~~~"]: "medium",
  [process.env.POLAR_LARGE_MONTHLY_PRODUCT_ID ?? "~~~~~"]: "large",
  [process.env.POLAR_LARGE_YEARLY_PRODUCT_ID ?? "~~~~~~"]: "large",
};
...
const mediumYearly = await db.subscription.count({
...
const largeYearly = await db.subscription.count({
