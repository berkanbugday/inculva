...
export function healthDegradedTemplate(userName: string, domain: string, siteId: string): string {
  return baseTemplate(
    `
    <h2>Widget may be offline on ${escapeHtml(domain)}</h2>
    <p>Hi ${escapeHtml(userName || "there")},</p>
    <p>Our health monitor couldn't detect the Inculva widget on <strong>${escapeHtml(domain)}</strong>. This might mean:</p>
    ...
    <a href="${APP_URL}/dashboard/sites/${escapeHtml(siteId)}" class="btn">Check site settings →</a>
    <hr class="divider">
    <p class="small">
